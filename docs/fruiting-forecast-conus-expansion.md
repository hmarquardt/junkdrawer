...
- Verified start: Revision 15 closed at `a0e02c9`; Batch-2 tooling was committed at `986729ea14454b00a59a5b282761dec705b04edd` two commits later (one unrelated Junkdrawer page plus the Batch-2 runner). No Fruiting Forecast production file changed between `a0e02c9` and the scope freeze; the difference is recorded rather than assumed away.
- National four-layer coverage before Batch 2: **336 / 940**.

### State preparation

- Requested cohort: 19 states — AL, AR, IA, IL, IN, KS, KY, LA, MN, MO, MS, NC, NE, OH, SC, TN, VA, WI, WV.
- Ready after preparation: **19 / 19**; verified caches reused: 1 (Alabama soil was already READY; every other state-level prerequisite was prepared in this pass).
- Measured preparation time: **9,123.1 s** total (SDA soil plus Geofabrik PBF download, validation and osmium extraction per state, strictly serialized). Duration is dominated by PBF downloads and extraction; soil attribute queries are seconds each.
- Restartable and idempotent: a READY state revalidates and reuses; a failed refresh never overwrites verified bytes.

### Frozen Batch 2

- Planner-derived, frozen in `production/batch2-scope.json`: **279 tiles** (eligible 615, already complete 336).
- Derivation criteria: every incomplete tile whose required states are all READY for both soil and OSM preparation. The planner, not a hand list, is authoritative.
- Revision 15 estimated 309. The 279-tile result differs because 30 incomplete tiles have no state at or above the planner 5% source threshold; the production runner cannot resolve soil states for such tiles, so they were never buildable in this pass. Criteria were not adjusted to reproduce 309.

### Execution

- Two concurrent local/DEM tile workers (`tools/fruiting_batch2.py`), one serialized publisher lane, 28 checkpoints of ten tiles or fewer.
- Strictly serialized: SDA and all hosted per-tile services (PAD-US and MTBS ArcGIS), R2 publication, manifest mutation, publisher-ledger mutation, journal writes and git checkpoints. Only DEM download and local per-tile computation overlap.
- Newly complete: **279 / 279** frozen tiles; 0 remaining in the frozen set (none).
- Hosted-service retries observed: none (0 in-process retry events across the whole Batch-2 journal). One ArcGIS body-level 429 occurred before the pacing/retry fix; it killed a worker and hung the queue, the fix landed, and the tile rebuilt successfully with no further 429. No failed request produced neutral or invented evidence, and missing stayed missing.
- Two real defects surfaced under Batch-2 load and were fixed without touching biology: (1) an invalid source ring crashed the OSM access phase with a GEOS side-location conflict on a KY/TN boundary tile; the adapter now repairs topology at every load site (`make_valid`), which changes geometry validity only, never mapped evidence. (2) An ArcGIS-hosted quota response arrived as HTTP 200 with a body-level 429, killed a builder worker and hung the checkpoint queue on `work.join()`; the adapter now retries body-level 429/5xx with bounded backoff (honoring the service retry hint) on a paced hosted lane, and a failed tile is recorded and no longer hangs or kills the queue. Both fixes are regression-tested.
- Published checkpoints stayed resumable: each checkpoint was generated by the runner only after re-reading the manifest and confirming zero incomplete tiles in the chunk, then committed and pushed.

### Result

- National four-layer coverage: **615 / 940**; **325** remain.
- Exact remaining tiles for the next pass: **325** relevant (0 buildable with current preparation, 30 with no state above the source threshold, 295 blocked on unprepared states).
- **Batch 2 completion is not national completion: Fruiting Forecast is NOT nationally launch-ready.**

### R2

- Batch-2 uploads (publisher ledger since the first metrics event): **1116 objects / 55,771,492 bytes**; 1116 upload intents, 417 objects verified and reused without re-upload. Scope: publisher upload-intent ledger since the first Batch-2 metrics event; not a bucket listing.
- Active manifest: **2469 objects / 193,000,766 bytes**.
- Final remote audit: 2469 remotely valid, 0 missing, 0 invalid, 91 local-only (reported, not deleted), 74 publisher-ledger remote orphans; clean = True.
- Audit scope is precise: publisher upload-intent ledger; not an exhaustive R2 bucket listing. Wrangler cannot exhaustively list the bucket, so this is not a claim that the bucket contains no other objects.
- Publication transport remained the proven Wrangler OAuth path. No bucket-scoped R2 S3 credentials were present in the environment (checked only through normal environment mechanisms; no values printed, no secret search performed), so the S3-compatible uploader was neither required nor benchmarked.

### Performance

- Final authoritative pass wall time across checkpoints: **14,185.0 s** for 279 tiles (**50.8 s/tile**). The full since-first-attempt span was **26,537.2 s** (**95.1 s/tile**), including earlier interrupted generations, duplicate work and stopped time; it is not an execution rate.
- Batch 1 ran 301 tiles in 70,059 s wall (15.5 tiles/hour including stopped time) at 93.7 s/tile of fully serial stage time. Batch 2 completed 70.8 tiles/hour in the final pass and 37.8 tiles/hour since first attempt.
- Resumed pass (chunks 10-27, the contiguous run that finished the list): 179 tiles in 14,188 s (45.4 tiles/hour); per-tile stage-sum median 136.7 s; **serial-stage speedup 1.84x** (sum of per-tile stage seconds / wall seconds); two-worker build utilization 0.921; publisher utilization 0.182; 169 DEM downloads / 8,534,994,465 bytes in 17,693 s; 0 retry events.
- DEM median 89.9 s/tile (p75 115.3) vs Batch-1 serial median 66.0 s/tile. Access median 17.6 s/tile (p75 32.3) vs Batch-1 2.8 s/tile: dense eastern OSM access computation is materially heavier than the western Batch-1 tiles.
- Concurrency effectiveness: Batch 2 completed 279 tiles with a final-pass chunk wall of 14,185.0 s and a since-first-attempt span of 26,537.2 s (95.1 s/tile including restarts vs 93.7 s/tile serial stage time in Batch 1). Worker build utilization 0.71; publisher utilization 0.176. DEM median 89.9 s/tile vs Batch-1 66.0; access median 17.6 s/tile vs Batch-1 2.8.
- Bottleneck analysis: DEM remains a major local constraint at 67.5% of build-busy time; dense eastern OSM access computation is the other at 23.6%, with access p75 32.3 s/tile far above the Batch-1 western median. Publication runs concurrently and did not become the limiter.
- R2 transport: Wrangler OAuth (no bucket-scoped S3 credentials present in the environment); 1116 objects / 55,771,492 bytes uploaded in this batch, 417 objects verified and reused without re-upload, publish lane busy 4,677.4 s (0.176 utilization). Publication is serialized and overlaps the two local workers; its share of wall time is reported rather than assumed. The S3-compatible path was neither required nor benchmarked because no bucket-scoped credentials were available.
- Resource floors: minimum free disk 5,797,072,896 bytes across the whole Batch-2 journal (observed during the interrupted earlier generations, which ran concurrently); the final authoritative resumed pass never went below 10,387,210,240 bytes. Peak DEM cache 2,669,024,709 bytes; DEM downloads 279 / 14,420,831,740 bytes; DEM bytes reclaimed 14,420,831,740. The documented minimum-free-space guards (8 GiB preflight, 4 GiB stop) stayed in force and were never lowered or triggered.

### Projections

- National storage across 615 complete tiles: mean 311,706 bytes/tile, median 210,290, p25 118,461, p75 380,398, p90 648,750. Layer totals: habitat 8.98 MB, public land 61.49 MB, fire 101.25 MB, access 21.19 MB. Profile-weighted national projection **208.31 MB**; simple-mean 219.44 MB.
- Remaining runtime (the 295 state-blocked tiles that become buildable once the 17 unprepared states are prepared): two-worker expectation **6.15 h** (p75 stages 8.24 h), serial-equivalent 10.62 h. Components: DEM 3.68 h, hosted 0.70 h, local compute 0.79 h, publication 0.98 h; state preparation for 17 states is estimated separately at 2.27 h from the measured Batch-2 per-state mean. State preparation is estimated separately from the measured Batch-2 per-state mean and is not included in the two-worker expectation; stopped/operator time and the no-state-threshold tiles are excluded.
- R2 cost model (assumptions unchanged and explicit): Assumptions unchanged from Revision 15 (4 searches x 9 tiles x 4 objects, 50% warm-cache avoidance). R2 free allowances are account-shared; egress free. Projected national storage 0.208 GB (profile-weighted) / 0.219 GB (simple mean); ~72 Class B reads/user/month gives 7,200 / 72,000 / 720,000 reads at 100 / 1,000 / 10,000 monthly users, all inside the account-shared free allowances in isolation. Edge caching was not configured and was not assumed; `CF-Cache-Status: DYNAMIC` remains acceptable and no Worker is warranted.

### Batch-3 recommendation

- Next pass would be **National GIS Batch 3**: 295 state-blocked tiles (0 buildable until state preparation runs), requiring preparation of 17 additional states (CT, DE, MA, MD, ND, NH, NJ, NM, NV, NY, OK, PA, RI, SD, TX, UT, VT). Operational grouping: the same ten-tile checkpoint cadence (about 30 checkpoints), state preparation strictly before any build.
- Two workers are recommended again (the same bounded two-worker scheduler, one serialized hosted lane, one serialized publisher). Measured bottleneck: DEM remains a major local constraint at 67.5% of build-busy time; dense eastern OSM access computation is the other at 23.6%, with access p75 32.3 s/tile far above the Batch-1 western median. Publication runs concurrently and did not become the limiter.
- Batch-2 evidence argues for keeping the phased A/B execution but measuring an A/B pipeline in Batch 3: phase A is DEM/network-bound while phase B is CPU-bound OSM access, so the two phases currently alternate rather than overlap. This is a recommendation for measurements, not a change made in this pass.
- No R2 transport change is justified without measurement; Wrangler remained correct, serialized and remote-verified on every object.
- The 30 no-state-threshold tiles are a planner/runner design question, not a data-preparation task, and are documented in `production/batch3-scope.json` rather than silently excluded.
...
The next possible production pass is **National GIS Batch 3**. Revision 16 does **not** authorize it and does not imply it has started. Batch 2 only is complete.
