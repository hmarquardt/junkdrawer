...
- tests/test_fruiting_bulk_adapters.py: 32 deterministic tests for adapter contracts, the cache manifest/checksum/corruption path, the Soil Data Access and package soil paths (normalized contract, batched point join, ambiguity, explicit failure/empty), DEM release resolution, habitat composition with optional sources absent, legacy-tile backward compatibility, and the committed bounded release (component/layer completeness, real SSURGO values, explicit soil gaps, release-scope manifest assertions).
...
3. gSSURGO/gNATSGO state packages are supported through the same normalized contract but were not obtainable from this environment (NRCS Box links 404, GDG 403, Web Soil Survey cart needs a session). The SDA path is the verified production source; a future pass can validate a package drop-in.
