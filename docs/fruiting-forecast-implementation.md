...
The 0.05° preparation grid (400 cells per 1° tile for newer tiles, up to 400 for earlier ones) is a regional screening surface, not 30 m local precision. The 40 checked-in tiles span approximately 506 KB of habitat data and 671 KB of public-land data across the Ohio Valley / Tennessee Valley corridor (35–40° N, 91–83° W). A Crossville 50-mile analysis selects four tiles (about 48 KB habitat, 2 public-land tiles). A Princeton 50-mile analysis selects nine tiles (about 110 KB habitat, 7 public-land tiles). The manifest preserves native source resolution separately from sample spacing.
...
### Public access / parking points

`data/fruiting-forecast/access-points.parquet` answers the next practical question — *where do I actually park or enter?* Pins are **access evidence, never mushroom locations**, and the UI repeats this on every pin popup.

- **Source hierarchy.** Tier 1 (authoritative agency trailhead/parking GIS) is preferred, but no stable machine-readable agency parking service exists for Indiana DNR / Hoosier NF / USFWS today, so the shipped dataset is Tier 2: OpenStreetMap access features (`amenity=parking`, `highway=trailhead`, `amenity=boat_ramp`, publicly passable `barrier=gate`, and visitor information nodes), fetched offline via Overpass in small sub-bboxes with mirror rotation. Tier-1 sources can be added later in `build_access_points()` without any UI change. OSM data is © OpenStreetMap contributors (ODbL); the attribution is retained in the manifest descriptor and the map note.
- **Association.** OSM points are matched to the same deduplicated PAD-US named properties used by the public-lands layer: point-in-polygon wins, and the smallest-area property wins when polygons nest. A point outside every boundary is only kept when it is an explicit access feature (trailhead/gate/parking) within ~130 m of a boundary, and it is then labeled LOW confidence. Private properties never receive pins. Points tagged `access=private/no/customers` are rejected outright. Nearby duplicates are suppressed on an ~80 m grid per property and type.
- **Confidence.** `HIGH` = the point's name/operator names the managing agency or the property; `MEDIUM` = a mapped access feature inside the property boundary; `LOW` = adjacent access intent near the boundary. Property centroids are never used to synthesize pins: a property with no reliable point shows **"No verified access point found"** instead.
- **Habitat distance.** Each property's strongest sampled habitat cell (forest cover + oak/hickory + canopy, deterministic) becomes an anchor; every pin reports its straight-line distance to that anchor and is explicitly labeled straight-line, not trail distance.
- **Suggested Start.** For each property the highest-confidence pin closest to the strongest habitat is surfaced as a planning suggestion in the ranking cards and property detail. It never feeds Biological Opportunity, Huntability, or the Recommended Hunt Score.
- **Static format.** Ordinary Parquet (no geometry column needed): `access_id, property_id, property_name, name, lat, lon, type, source, source_url, confidence, official, operator, notes, verified_at`. Types normalize to `PARKING, TRAILHEAD, HUNTER_ACCESS, PUBLIC_ENTRANCE, GATE, BOAT_RAMP, VISITOR_AREA, OTHER_ACCESS`. The browser loads it through the same cached `gisAssetBytes` + DuckDB-Wasm path as the other GIS assets, degrading gracefully (huntability and biology unaffected) if the file is missing.
- **Refresh.** `tools/build-fruiting-gis.py --access-points-only` re-fetches OSM access features (cached per sub-bbox), re-associates against the PAD-US cache, and updates `manifest.accessPoints` (dataset version, byte count, SHA-256, attribution). Re-run seasonally; OSM coverage of forest access points improves continually, and the `verified_at` field records freshness.

### Theme architecture

The stylesheet is fully token-driven: every surface, border, and text color resolves through semantic CSS custom properties defined once in `:root` (dark values) and overridden in `html[data-theme="light"]`. A no-flash inline script applies the stored preference before first paint. The `themeSelect` control (Light / Dark / System, default System) persists to the same small localStorage preferences record, follows `prefers-color-scheme` changes while in System, and re-renders the Leaflet map on change because map stroke/fill colors are resolved from the live token values. Dark-mode tile dimming (brightness/saturation filter) applies only under `data-theme="dark"`; light mode uses near-native tiles with a mild saturation trim. Light-mode targets: body text ≥ 7:1, muted/labels ≥ 4.5:1 (WCAG AA), no pale-on-pale combinations; dialogs, forms, score bands, the analyst module, and the map legend were audited in both themes at desktop and 390×844 mobile widths.
