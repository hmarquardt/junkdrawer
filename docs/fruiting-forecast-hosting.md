...
Phase 1B completed the provider-neutral asset-base integration, migrated and verified all 250 pre-cutover live objects, removed the production Parquet dependency on GitHub Pages, and completed the planner-derived 301-tile Batch 1. The active manifest now references 1,427 R2 objects / 138,601,703 bytes. A final full GET/length/SHA-256 audit validated every reference with zero missing or corrupt objects and zero remote orphans in the publisher ledger. Real deployed Chrome/DuckDB-Wasm cold/warm tests passed across six newly completed biological regions, and the full browser and Python suites are recorded in the revision-15 handoff.

Production Parquet is ignored by git and can be hydrated from R2 for release-data tests. Manifest/config/provenance remain versioned. Existing unrelated worktree files were preserved. The next authorized production pass is **National GIS Batch 2 only**; it must rerun preflight and the planner before freezing scope. Batch 3 and the full remainder are not authorized by this handoff.
