...
function prominence(p){
 if(p.train){const s=p.train.state;
  return {points:s==='Fresh Train'?4:s==='Dispersing Train'?3:2,
   label:s==='Fresh Train'?'Fresh Starlink train; formations like this spread within days':s==='Dispersing Train'?'Recognizable dispersing Starlink train':'Faint remaining train structure; subtle observation'};}
...
function classifyEventQuality(p,ctx,relative){
 ...
 if(p.score<90)downgrades.push('Sighting quality below 90');if(p.train?.stale)downgrades.push('Train orbital elements are stale; score capped and confidence reduced');
 ...
 if(!downgrades.length){label='Exceptional';reasons.push('High-quality, high, sustained pass under clear dark skies',relative.sampleCount<5?'Exceptional absolute geometry; comparison sample is small':'Stands out within the selected seven-night observations');}
