...
test('ISS 8:17 PM pass must not disappear at 8:00 PM',async({page})=>{
 const errors=await boot(page),{fixture,malformed,now}=require('./overhead-lifecycle.cjs');
 await page.clock.setFixedTime(new Date(now));
 await page.evaluate(({fixture,now})=>{
  const s=__OVERHEAD_TEST__.state;s.worker?.terminate();s.busy=false;s.loading=false;s.site={...s.site,tz:'America/Chicago'};s.nights=OverheadEngine.nights(s.site,now);s.windows=[...s.nights,{start:now,end:now+7200000}];s.day=0;s.view='tonight';
  const next=structuredClone(fixture);next.id='next-night';next.score=75;for(const k of ['rise','start','end','set'])next[k]+=86400000;for(const k of ['peak','orbitalPeak','entry','exit'])next[k].t+=86400000;
  s.results=Object.fromEntries(s.nights.map((_,i)=>[i,i===0?[fixture]:i===1?[next]:[]]));s.selected=fixture;__OVERHEAD_TEST__.render();window.__lifecycleResults=s.results;
 },{fixture,now});
 const event=page.locator('[data-event="'+fixture.id+'"]');await expect(event).toContainText('8:17 PM');await expect(event).toBeVisible();
 await event.click();const dialog=await page.locator('#detail-content').innerHTML();await expect(page.locator('#detail-content .sky')).toBeVisible();
 for(let minute=0;minute<=34;minute++){
  await page.clock.setFixedTime(new Date(now+minute*60000));await page.evaluate(()=>__OVERHEAD_TEST__.render());
  expect(await page.evaluate(()=>__OVERHEAD_TEST__.state.selected.id)).toBe(fixture.id);await expect(page.locator('#event-dialog')).toBeVisible();expect(await page.locator('#detail-content').innerHTML()).toBe(dialog);
  expect(await page.evaluate(()=>__OVERHEAD_TEST__.state.weekly.winner?.pass.id)).toBe(minute<23?fixture.id:'next-night');await expect(event).toHaveCount(1);
  if(minute>=23)await expect(event).toContainText('JUST ENDED');else if(minute>=17)await expect(event).toContainText('IN PROGRESS');
  expect(await page.evaluate(()=>__OVERHEAD_TEST__.state.results===__lifecycleResults)).toBe(true);
 }
 await page.keyboard.press('Escape');await event.click();await expect(page.locator('#detail-content .sky')).toBeVisible();await page.keyboard.press('Escape');
 await page.clock.setFixedTime(new Date(now+35*60000));await page.evaluate(()=>__OVERHEAD_TEST__.render());await expect(event).toHaveCount(0);
 await page.clock.setFixedTime(new Date(now));await page.evaluate(p=>{const s=__OVERHEAD_TEST__.state;s.results[0]=[p];s.selected=p;__OVERHEAD_TEST__.render();},malformed);
 await expect(event).toBeVisible();await expect(event).toContainText('TIMING WARNING');expect(await page.evaluate(()=>__OVERHEAD_TEST__.state.weekly.winner.pass.id)).toBe(fixture.id);await expect(page.locator('#diagnostic-output')).toContainText('TIMING INVARIANT WARNING');await event.click();await expect(page.locator('#detail-content .sky')).toBeVisible();expect(errors).toEqual([]);
});
...
test('theme-only rendering, deployed dark parity, contrast and persistence',async({page,browser})=>{
 ...
 const server=http.createServer((req,res)=>{const raw=new URL(req.url,'http://local').pathname;const file=raw.replace(/^\/baseline/,'').slice(1);if(file==='overhead.html'&&raw.startsWith('/baseline/')){res.setHeader('Content-Type','text/html');res.end(baseline);return;}if(!['overhead.html','overhead-engine.js','overhead-weekly.js','overhead-lifecycle.js','overhead-trains.js','overhead-objects.js','overhead-worker.js','analytics-lite.js','vendor/overhead/satellite-6.0.1.min.js','vendor/overhead/suncalc-1.9.0.js'].includes(file)){res.writeHead(404);res.end();return;}res.setHeader('Content-Type',file.endsWith('.html')?'text/html':'application/javascript');res.end(fs.readFileSync(path.resolve(file)));});
